...
Before running the empirical notebooks, download and prepare the inputs listed in Sections 2 and 6, create the required directories, and install the environment described in Section 4. Open Jupyter from the package root and run the notebooks in this order.
...
The structure below shows the paths expected after downloading and preparing the inputs. It is not a list of datasets distributed with the submission.
...
`results/figures` contains the figures cited in the report together with the supporting TDABM colour maps and robustness figures generated by notebooks 02 and 04. `results/eda_tables` and `results/tdabm_tables` contain machine-readable versions of the reported summaries and robustness outputs. The simplified GeoPackage created during mapping is a cache and is deliberately omitted because notebooks 01 and 03 can regenerate it from the prepared shapefile.
...
Before and after running the empirical notebooks, check the following. The reported row counts refer to the source versions used for the report:
...
For a clean verification, copy the folder to a new location, create the environment, run the five empirical notebooks in order, run the method-figure notebook separately, and compare generated filenames and summary counts with the report and any retained reference tables. A manifest describes the local files from which it was generated and should be refreshed if the submission contents change.
